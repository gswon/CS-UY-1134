    ...
    def leaves_list(self):
        def helper(node1):
            ...
            if node1.left is None and node1.right is None:
                yield node1.data
            elif node1.left is None and node1.right is not None:
                yield from helper(node1.right)
            elif node1.right is None and node1.left is not None:
                yield from helper(node1.left)
            else:
                yield from helper(node1.left)
                yield from helper(node1.right)
